...
filepath=$(sed -n "s/.*filepath *= *'\([^']*\)'.*/\1/p" /home/biqu/printer_data/config/saved_variables.cfg)
filepath=$(printf "$filepath")
echo "filepath=$filepath"
last_file=$(sed -n "s/.*last_file *= *'\([^']*\)'.*/\1/p" /home/biqu/printer_data/config/saved_variables.cfg)
last_file=$(printf "$last_file")
echo "$last_file"
plr=$last_file

height=$(jq -r '.Z' /home/biqu/sovol_plr_height)
commandline=$(jq -r '.commandline' /home/biqu/sovol_plr_height)
extrude_type=$(jq -r '.extrude_type' /home/biqu/sovol_plr_height)
e_extrude_abs=$(jq -r '.e_extrude_abs' /home/biqu/sovol_plr_height)
...
echo "$file_content" | sed  's/\r$//' | awk -F"Z" 'BEGIN{OFS="Z"} {if ($2 ~ /^[0-9]+$/) $2=$2".0"} 1' > /home/biqu/plrtmpA.$$

echo "$comment_content" > ${PLR_PATH}/"${plr}"
grep ';TIME:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';Layer height:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';MINX:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';MINY:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';MINZ:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';MAXX:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';MAXY:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';MAXZ:' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
grep ';Generated with Sovol Slicer' /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
echo "SET_KINEMATIC_POSITION Z=$height" >> ${PLR_PATH}/"${plr}"

# BG_EX=`tac /home/biqu/plrtmpA.$$ | sed -e '/$commandline\$/q' | tac | tail -n+2 | sed -e '/ Z[0-9]/ q' | tac | sed -e '/ E[0-9]/ q' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p'`
# echo 'G92 E0.0' >> ${PLR_PATH}/"${plr}"
# echo "$BG_EX" 
# if [ "${BG_EX}" = "" ]; then
#     BG_EX=`tac /home/biqu/plrtmpA.$$ | sed -e '/$commandline\$/q' | tac | tail -n+2 | sed -ne '/ Z/,$ p' | sed -e '/ E[0-9]/ q' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p'`
# fi
...
cat /home/biqu/plrtmpA.$$ | sed '/ Z'${height}'/q' | sed -ne '/\(M104\|M140\|M109\|M190\|M106\)/p' >> ${PLR_PATH}/"${plr}"
...
#复制后续gcode
sed -n "/$commandline/,\$p" /home/biqu/plrtmpA.$$ >> ${PLR_PATH}/"${plr}"
